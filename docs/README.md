...
| [`aios-architecture-and-phases.md`](aios-architecture-and-phases.md) | **Primary reference** — lifecycle flow, Phases 1–34 design (TOC with anchors), API/DB index (Phase 21), the Forward Plan's own restructure review and sequencing rationale (merged in above Phase 25), deployment, backup |
...
| `services/agents/agents/reasoning_engine/django_bridge.py` (`DJANGO_PYTHON_BIN`) + `services/execution/execution/shell_executor/sandbox.py` (`SANDBOX_MEMORY_LIMIT_MB`) | 34 | [`aios-architecture-and-phases.md#phase-34-real-external-project-integration`](aios-architecture-and-phases.md#phase-34-real-external-project-integration) | yes (first real external project connected — a real project's own venv interpreter, and an explicit, default-preserving opt-in past the sandbox's 512MB memory cap, both found live against a real Django + pandas codebase, not a disposable test project) |
