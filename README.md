...
| 32 | Schema Drift Detection | [`docs/aios-architecture-and-phases.md#phase-32-schema-drift-detection`](docs/aios-architecture-and-phases.md#phase-32-schema-drift-detection) | A real gap found while correcting a stray, factually-incorrect planning draft ([`docs/ARCHITECTURE_PLAN.md`](docs/ARCHITECTURE_PLAN.md)). [`services/knowledge_pipelines/knowledge_pipelines/erp_knowledge_engine/drift.py`](services/knowledge_pipelines/knowledge_pipelines/erp_knowledge_engine/drift.py) — a real structured table/column diff against the stored schema snapshot, `GET .../drift` (read-only check) + `POST .../check-and-sync` (only re-syncs on genuine drift), explicit and on-demand, never a background daemon |
| 33 | Operating Discipline | [`docs/aios-architecture-and-phases.md#phase-33-operating-discipline`](docs/aios-architecture-and-phases.md#phase-33-operating-discipline) | A user-supplied operating manual, applied to two different audiences at two different token budgets: [`.cursor/rules/operating-discipline.mdc`](.cursor/rules/operating-discipline.mdc) (near-verbatim, governs Claude Code/Cursor on this repo) and a short, distilled addendum to [`services/assembly/assembly/prompt_builder/shared_fragments.py`](services/assembly/assembly/prompt_builder/shared_fragments.py)'s one shared fragment every one of 25+ agents already embeds — reaching every agent's rendered prompt with zero per-agent template edits, deliberately kept short to respect Phase 25's own measured finding that verbose prompting hurts the local model's structured-output reliability |
| 34 | Real External Project Integration | [`docs/aios-architecture-and-phases.md#phase-34-real-external-project-integration`](docs/aios-architecture-and-phases.md#phase-34-real-external-project-integration) | The first real external, live business codebase connected (not a disposable test project). [`django_bridge.py`](services/agents/agents/reasoning_engine/django_bridge.py)'s interpreter is now configurable (`DJANGO_PYTHON_BIN`) for a real project's own venv; a second, real structural finding — the sandbox's 512MB memory cap was too low for a real Django+pandas import chain, confirmed by direct reproduction — got a real, explicit, default-preserving opt-in (`SANDBOX_MEMORY_LIMIT_MB`), never a silent global raise |
