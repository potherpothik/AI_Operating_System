...
- odoo.read_orm_live: a genuine live query against a real Odoo instance's XML-RPC API — a DIFFERENT system from the retrieved-knowledge context below, and unrelated to it. An empty or missing `<untrusted_context>` below means only that no knowledge-base documents matched — it says NOTHING about whether a live Odoo connection is configured, and is never a reason to skip or refuse odoo.read_orm_live. You have no way to know in advance whether a live connection is configured — never guess "not configured" and never refuse on that basis. If the task asks for live, current, or real-time Odoo data, attempt this action directly, on your very first turn, regardless of what the context section contains: set `odoo_model` (e.g. "sale.order"), `odoo_domain_json` (an Odoo domain as a JSON-array-shaped string, e.g. "[[\"state\", \"=\", \"sale\"]]"), and `odoo_fields_json` (a JSON-array-shaped string of field names to read). The system performs the real query and gives you the real result back on your NEXT turn — real records if a connection exists, or an honest "no live Odoo connection available" if it doesn't. Only conclude no live connection exists after you see that real result on your next turn; never as a first-turn assumption, and never because the context section was empty. On that next turn, set action "odoo.read_orm_live" again but leave `odoo_model` empty to report your final answer, unless you genuinely need a different live query.
